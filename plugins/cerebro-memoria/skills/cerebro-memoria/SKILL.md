...
Se qualquer script responder que o vault não está configurado (`"erro": "Vault do Cerebro nao configurado..."`), faça o setup **antes** de continuar a tarefa, e só então volte para o que a pessoa pediu:

1. Rode `configurar_vault.py --detectar` para listar os vaults que o Obsidian conhece nesta máquina.
2. **Pergunte qual usar** — mostre os candidatos com o caminho completo. Não escolha sozinho, mesmo que só apareça um chamado "Cerebro": escrever no vault errado é um estrago que só aparece semanas depois. Se a lista vier vazia (Obsidian não instalado, ou vault nunca aberto aqui), peça o caminho.
...
Resolvido o caminho, o vault tem (ou deveria ter) um arquivo `_config/config.md` com o comportamento desejado. Esse arquivo mora *dentro* do vault e controla o comportamento; não confunda com `~/.cerebro-memoria/config.json`, que só guarda **onde** o vault fica.
...
```bash
python "${CLAUDE_PLUGIN_ROOT}/skills/cerebro-memoria/scripts/configurar_vault.py"
python "${CLAUDE_PLUGIN_ROOT}/skills/cerebro-memoria/scripts/configurar_vault.py" --detectar
python "${CLAUDE_PLUGIN_ROOT}/skills/cerebro-memoria/scripts/configurar_vault.py" --vault "D:\Obsidian\Cerebro"
python "${CLAUDE_PLUGIN_ROOT}/skills/cerebro-memoria/scripts/configurar_vault.py" --vault "D:\Obsidian\Cerebro" --criar
python "${CLAUDE_PLUGIN_ROOT}/skills/cerebro-memoria/scripts/configurar_vault.py" --limpar
```
...
```bash
python "${CLAUDE_PLUGIN_ROOT}/skills/cerebro-memoria/scripts/buscar_notas.py" --query "sistema financeiro"
python "${CLAUDE_PLUGIN_ROOT}/skills/cerebro-memoria/scripts/buscar_notas.py" --tipo decisao_tecnica --projeto "Sistema Financeiro"
python "${CLAUDE_PLUGIN_ROOT}/skills/cerebro-memoria/scripts/buscar_notas.py" --tag sap --tag idoc
python "${CLAUDE_PLUGIN_ROOT}/skills/cerebro-memoria/scripts/buscar_notas.py" --pasta "01 - Projetos"
```
...
```bash
python "${CLAUDE_PLUGIN_ROOT}/skills/cerebro-memoria/scripts/inicializar_vault.py"
```
