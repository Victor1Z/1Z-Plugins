...
Os scripts em `scripts/` esperam Python 3 disponível no ambiente onde você está rodando, e recebem o caminho do vault via `--vault`.
...
3. **Procure antes de criar.** Rode `scripts/buscar_notas.py` filtrando por tipo/projeto/tags para ver se já existe uma nota sobre esse mesmo assunto. Isso é o passo mais importante do fluxo — pular ele é a causa nº 1 de vault bagunçado.
4. **Decida atualizar vs. criar:**
   ...
   - Não achou nada e a classificação está clara → **crie** a nota usando o template correspondente (`references/templates.md`).
...
5. **Preserve histórico.** Ao atualizar uma nota, não sobrescreva informação antiga — adicione à seção `## Histórico` com a data, e só então atualize o corpo principal se o fato mudou (ex: decisão trocada). Um exemplo do padrão está em `references/regras.md`.
...
1. Rode `scripts/buscar_notas.py` com a query e, se souber, filtros de tipo/projeto/tag.
...
- `references/estrutura-vault.md` — árvore de pastas completa e o que vai em cada uma.
- `references/templates.md` — template de frontmatter + corpo para projeto, reunião, decisão técnica, incidente e conhecimento técnico. Use como base ao criar notas novas; os nomes dos campos de frontmatter aqui são os que `buscar_notas.py` espera para filtrar (`tipo`, `projeto`, `tags`, `status`, `data`/`data_evento`/`data_criacao`).
- `references/regras.md` — as regras completas de organização, com o porquê de cada uma (duplicação, exclusão, Inbox, etc.).
...
python scripts/buscar_notas.py --vault "..." --pasta "01 - Projetos"
```
